...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.model == 'DE_I-AutoRec':
            model = DE_IAutoRec(sess, n_user, n_item, lr_min=0, lr_max=0.001, reg_rate_min=0, reg_rate_max=1)
            model.build_network()
            model.execute(train_data, validation_data, test_data)
        ...
        if args.model == 'DE_NRR':
            model = DE_NRR(sess, n_user, n_item, lr_min=0, lr_max=0.0001, reg_rate_min=0, reg_rate_max=1)
            model.build_network()
            model.execute(train_data, validation_data, test_data)
