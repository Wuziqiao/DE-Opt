...
class NGCF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        """
        # 经过ngcf等模型之后可以得到user和正负例的lookup最后的嵌入
...
if __name__ == '__main__':
    ...
    for i in [0.0001]:
        for j in [0, 0.001, 0.01, 0.1, 1, 10]:
            ...
            *********************************************************
            ...
            for epoch in range(args.epoch):
                ...
                # # 计算bpr的loss
                # model_mf_loss, model_emb_loss, model_reg_loss = model.create_bpr_loss(reg_rate, model.u_g_embeddings, model.pos_i_g_embeddings, model.neg_i_g_embeddings)
                # model_loss = model_mf_loss + model_emb_loss + model_reg_loss
                # 优化器的选择
                # model_opt = tf.train.GradientDescentOptimizer(learning_rate=lr).minimize(model_loss)
                # model_opt = tf.train.AdamOptimizer(learning_rate=lr).minimize(model_loss)
                ...
                NDCG_10 = ret_validation['ndcg'][0]
                NDCG_20 = ret_validation['ndcg'][1]
                RECALL_10 = ret_validation['recall'][0]
                RECALL_20 = ret_validation['recall'][1]
                t2 = time()
                if NDCG_10 > max_NDCG:
                    max_NDCG = NDCG_10
                    min_round = epoch + 1
                    ...
                    ndcg_test_10 = ret_test['ndcg'][0]
                    ndcg_test_20 = ret_test['ndcg'][1]
                    recall_test_10 = ret_test['recall'][0]
                    recall_test_20 = ret_test['recall'][1]
                    if ndcg_test_10 > best_NDCG_10:
                        best_NDCG_10 = ndcg_test_10
                        best_NDCG_20 = ndcg_test_20
                        best_RECALL_10 = recall_test_10
                        best_RECALL_20 = recall_test_20
                        t3 = time()
                if (epoch - min_round) >= delay_round:
                    break
                print("Epoch: %04d; " % (epoch + 1),
                      "NDCG@10: %.6f" % NDCG_10 + "; NDCG@20: %.6f" % NDCG_20 + "; RECALL@10: %.6f" % RECALL_10 + "; RECALL@20: %.6f" % RECALL_20,
                      '; time: %.2f' % (t2 - t1))
            print('Result on testset:\n', 'NDCG@10: %.6f' % best_NDCG_10, '; NDCG@20: %.6f' % best_NDCG_20,
                  '; RECALL@10: %.6f' % best_RECALL_10, '; RECALL@20: %.6f' % best_RECALL_20, '; timeCost:%.2f' % (t3 - t0))
