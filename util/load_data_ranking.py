...
def load_data_all(path="../data/ml1m/1m_ratings.dat", header=['user_id', 'item_id', 'rating', 'time'],
                  test_size=0.2, sep="::"):
...
def load_data_neg(path="./data/ml1m/1m.dat", header=['user_id', 'item_id', 'rating', 'category'],
                  test_size=0.2, sep="::"):
    df = pd.read_csv(path, sep=sep, names=header, engine='python')
    ...
    validation_dict = {}
    for u in range(n_users):
        validation_dict[u] = validation_matrix.getrow(u).nonzero()[1]

    print("Load data finished. Number of users:", n_users, "Number of items:", n_items)
    return train_matrix.todok(), validation_dict, test_dict, n_users, n_items
